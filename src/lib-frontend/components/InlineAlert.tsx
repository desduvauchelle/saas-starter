...
const InlineAlert: FC<{
	children: ReactNode,
	type?: "info" | "success" | "warning" | "error",
	detailed?: { title: string, message: string },
	buttons?: ReactNode
}> = ({ children, type, detailed, buttons }) => {
	...
	return <div className={className}>
		...
		<div>
			{children}
			{detailed && <>
				<h3 className="font-bold">{detailed.title}</h3>
				<div className="text-xs">{detailed.message}</div>
			</>}
		</div>
		{buttons}
	</div>
